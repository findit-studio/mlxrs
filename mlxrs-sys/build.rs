...
fn main() {
  ...
  // Phase 0 (this commit) ships a hand-written FFI subset in `src/lib.rs`
  // that exercises the link chain. Phase 2 will replace it with a pre-committed
  // bindings.rs generated by `cargo run -p xtask -- regen-bindings`, and add
  // an optional buildtime regen path here under `cfg(feature = "buildtime_bindgen")`.
}
