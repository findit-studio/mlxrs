...
//!
//! # Bench results (M2 Pro, release, 2026-05-24)
//!
//! Generated by `mlxrs/benches/simd_lfilter.rs` with `--warm-up-time 1
//! --measurement-time 3` (criterion); median throughput per group.
//!
//! ## Biquad lane-sweep (single-stage HS coefficients @ 48 kHz)
//!
//! ```text
//! n       generic         biquad_scalar   biquad_dispatch
//! 1024    465 Melem/s     506 Melem/s     506 Melem/s
//! 4096    466 Melem/s     499 Melem/s     528 Melem/s
//! 16384   476 Melem/s     523 Melem/s     521 Melem/s
//! 48000   473 Melem/s     525 Melem/s     530 Melem/s
//! 65536   477 Melem/s     527 Melem/s     ~470 (variance)
//! ```
//!
//! ## K-weighting chain (HS → HP @ 48 kHz × 1 s, the actual workload)
//!
//! ```text
//! variant            time            throughput
//! generic            201 µs          239 Melem/s
//! biquad_scalar      178 µs          269 Melem/s     ~1.13× over generic
//! biquad_dispatch    178 µs          270 Melem/s     ~1.13× over generic
//! ```
//!
...
//! 1024    12.5 Gelem/s    7.4 Gelem/s     NEON SLOWER (autovec wins)
//! 4096    8.9 Gelem/s     7.2 Gelem/s     NEON SLOWER
//! 16384   6.5 Gelem/s     7.7 Gelem/s     NEON slightly faster
//! 48000   10.3 Gelem/s    7.8 Gelem/s     NEON SLOWER
//! 65536   10.4 Gelem/s    7.8 Gelem/s     NEON SLOWER
//! ```
//!
...
//! the "SIMD ship NEON regardless" rule):
//!
//! - **SHIP biquad specialization** — beats generic by ~1.13× at the
//!   actual K-weighting workload (48 kHz chain, the only size that
//!   matters for [`crate::audio::dsp::integrated_loudness`]), AND
//!   preserves bit-exactness against the generic kernel (asserted by
//!   the `biquad_bit_exact_vs_generic_*` tests below over 48000-sample
//!   HS / HP / chained fixtures). Wired into both
//!   [`crate::audio::dsp`]'s `lfilter_f64` (out-of-place) and
//!   `lfilter_f64_in_place` (the in-place K-weighting consumer).
