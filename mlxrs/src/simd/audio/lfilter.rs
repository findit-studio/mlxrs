//! C9 — `lfilter` IIR recurrence (BS.1770 K-weighting hot loop).
//!
//! Tracking: [#154](https://github.com/Findit-AI/mlxrs/issues/154).
//! Plan: `docs/core-arch-simd-candidates.md` §2 row C9. Originally
//! documented as **NON-CANDIDATE** in `simd::audio` because the IIR
//! recurrence has a loop-carried dependency (every `y[n]` depends on
//! `y[n-1]`, the next state cell on `y[n]`, etc. — no within-stream
//! parallelism), so a wide NEON kernel over the sample axis is
//! mathematically impossible without changing the algorithm.
//!
//! # Per user directive 2026-05-24 — try anyway, bench-driven
//!
//! User asked for an EMPIRICAL attempt. Three approaches were tried
//! (in order, easiest → research-grade):
//!
...
//!
//! # Bench results (M2 Pro, release, authoritative re-run 2026-05-24)
//!
...
//! number). Captured to `/tmp/c9-r2-bench-authoritative.txt` on the
//! R2-fix reconciliation run; the earlier in-source numbers were from
//! an earlier compatible run on the same workload, reconciled here to
//! the fresh authoritative run so the doc, the live dispatcher (in
//! `audio::dsp.rs`), and the bench output are all one set of values.
//!
...
//! ```
//!
//! IN-PLACE dispatch beats generic at every benched length on the
//! current run (+44% to +62%), including the realistic long-channel
...
//! documented below. The previously-stated "within criterion noise"
//! framing was inaccurate at that point and has been corrected.
//!
...
//! lengths on the current run (+30% to +53%), including 4 s and 10 s
//! @ 48 kHz long channels. This is the consumer that drives the ship
//! decision for the in-place arm — but see the cross-run stability
//! caveat below before treating the magnitude as settled.
//!
//! ## Cross-run baseline stability
//!
//! The numbers above are from the authoritative `feb477c` re-run. A
//! prior byte-identical-harness run at commit `6728548` produced
//! substantially different generic-loop baselines while dispatch
//! itself drifted by single-digit percent across most rows (and
//! ~+11% on one in-place row). Row-accurate prior vs current
//! dispatch + generic values:
//!
//! ```text
...
//! ```
//!
//! Generic baselines dropped roughly 21-35% across both arms between
//! the two runs (in-place rows: 27.6% to 34.6%; K-weight rows: 20.9%
//! to 31.9%). Dispatch values themselves also varied across the
//! runs (e.g. in-place at `n=1024` moved from 487 → 543 Melem/s,
//! about +11.5%, and the K-weighting rows moved by roughly -3% to
//! -5% on each row), but dispatch consistently beat the generic
//! baseline in BOTH runs at every benched length. The mechanism for
//! the generic-baseline movement is not identified (likely a thermal,
//! scheduler, or codegen-side effect on a route we are not actively
//! tuning); the observable consequence is that the inflated current-
//! run ratios (+44% to +62% in-place, +30% to +53% K-weight) are
//! substantially driven by that baseline shift, not by a newly-
//! established dispatch improvement.
//!
//! ### Conservative lower-bound win (prior-run same-run ratios)
//!
//! The conservative ship basis below uses the prior `6728548` run's
//! own row-paired dispatch-vs-generic ratios — i.e. SAME-RUN observed
//! ratios from a single run, NOT a cross-run mixed envelope.
//! Reading the prior run's table row by row:
//!
//! - **IN-PLACE biquad dispatch beats generic by +3.0% to +4.7%**
//!   across the prior-run lane sweep (minimum +3.0% at n=16384;
//!   maximum +4.7% at n=1024). The current run shows +44% to +62%
//!   but those ratios should not be treated as settled until
//!   reproduced across independent runs.
//! - **K-weight chain dispatch beats generic by +8.1% to +9.4%**
//!   across the prior-run sweep (minimum +8.1% at n=48000; maximum
//!   +9.4% at n=480000). The current run's +30% to +53% is similarly
//!   contingent on the depressed generic baseline reproducing.
//!
//! ### Ship decision basis
//!
//! The KEEP-WIRED decision for the in-place arm is grounded on the
//! CONSERVATIVE lower-bound win read from a single prior run's own
//! row-paired ratios (+3.0% to +4.7% on the in-place lane sweep,
//! +8.1% to +9.4% on the K-weighting chain), not on the current
//! run's inflated numbers and not on a cross-run mixed envelope. A
//! few-percent win that survives across two independent runs at
//! every benched length is sufficient to KEEP wiring on the
//! K-weighting hot path; the larger ratios would be a nice-to-have
//! IF reproduced, but the ship decision does not depend on them.
//! The per-arm ship calls in the **Decision** section below follow
//! that conservative framing.
//!
...
//! # Decision
//!
//! Per user directive 2026-05-24 (C9 ship-decision IS the bench, not
//! the "SIMD ship NEON regardless" rule; "KEEP wiring only if benchmark
//! proves > scalar at the actually-wired paths; prefer revert when in
//! doubt"):
//!
//! - **SHIP + WIRE biquad specialization at the in-place path only**
//!   (`crate::audio::dsp::lfilter_f64_in_place`) — grounded on the
//!   conservative lower-bound win read from the prior `6728548`
//!   run's own row-paired ratios (+3.0% to +4.7% on the in-place
//!   lane sweep, +8.1% to +9.4% on the K-weighting chain; SAME-RUN
//!   ratios, not a cross-run mixed envelope). See the **Cross-run
//!   baseline stability** section above for the row-accurate
//!   prior/current tables. The current `feb477c` run shows
//!   substantially larger ratios (+44% to +62% in-place, +30% to
//!   +53% K-weight) but those are inflated by depressed generic
//!   baselines, not a newly-established dispatch improvement, and
//!   are not the basis of the ship decision.
...
//!   confident length-gating threshold. Per "prefer revert when in
//!   doubt", the out-of-place wrapper runs its single-pass generic
//!   loop (no extra `extend_from_slice` memcpy).
...
mod tests {
  ...
  /// Lane-sweep covers C9 FIR-relevant boundary lengths.
  #[test]
