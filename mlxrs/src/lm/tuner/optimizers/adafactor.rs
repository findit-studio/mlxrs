...
use crate::{
  Array, Result,
  error::{Error, InvariantViolationPayload, OutOfRangePayload},
  lm::{
    load::Weights,
...
fn validate_decay_rate(decay_rate: f32) -> Result<()> {
  if !decay_rate.is_finite() || decay_rate > 0.0 {
    return Err(Error::OutOfRange(OutOfRangePayload::new(
      "Adafactor: decay_rate (so 1 - step^decay_rate stays in [0, 1))",
      "must be a finite float <= 0.0",
...
fn validate_beta_1(beta_1: Option<f32>) -> Result<()> {
    ...
    && (!b.is_finite() || !(0.0..1.0).contains(&b))
  {
    return Err(Error::OutOfRange(OutOfRangePayload::new(
      "Adafactor: beta_1",
      "must be None or a finite float in [0.0, 1.0)",
      format!("Some({b})"),
    )));
  }
  Ok(())
  ...
  pub fn with_beta_1(mut self, beta_1: Option<f32>) -> Result<Self> {
    if !self.state.is_empty() {
      return Err(Error::InvariantViolation(InvariantViolationPayload::new(
        ...
        "cannot toggle beta_1 after parameter state is initialized (would desynchronize \
         existing vs new parameters' exp_avg shape); construct a fresh Adafactor or \
         use try_set_beta_1 (which preserves state on error)",
      )));
    }
    validate_beta_1(beta_1)?;
    self.beta_1 = beta_1;
  ...
  pub fn try_set_beta_1(&mut self, beta_1: Option<f32>) -> Result<()> {
    if !self.state.is_empty() {
      return Err(Error::InvariantViolation(InvariantViolationPayload::new(
        ...
        "cannot toggle beta_1 after parameter state is initialized (would desynchronize \
         existing vs new parameters' exp_avg shape); construct a fresh Adafactor instead",
      )));
    }
    validate_beta_1(beta_1)?;
    self.beta_1 = beta_1;
