...
fn parses_in_config_cmvn_pair() {
  // `input_size = 4` with a matching 4-wide means/istd pair validates.
  let json = r#"{
    "input_size": 4,
...
fn cmvn_length_mismatch_is_rejected() {
  // A 3-wide means against `input_size = 4` is a typed LengthMismatch.
  let json =
    r#"{ "input_size": 4, "cmvn_means": [0.1, 0.2, 0.3], "cmvn_istd": [1.0, 1.0, 1.0, 1.0] }"#;
  let c: Config = serde_json::from_str(json).expect("parse");
...
fn cmvn_means_without_istd_is_rejected() {
  let json = r#"{ "input_size": 2, "cmvn_means": [0.1, 0.2] }"#;
  let c: Config = serde_json::from_str(json).expect("parse");
  assert!(matches!(c.validate(), Err(Error::LengthMismatch(_))));
}
...
fn cmvn_istd_without_means_is_rejected() {
  let json = r#"{ "input_size": 2, "cmvn_istd": [1.0, 1.0] }"#;
  let c: Config = serde_json::from_str(json).expect("parse");
  assert!(matches!(c.validate(), Err(Error::LengthMismatch(_))));
}
