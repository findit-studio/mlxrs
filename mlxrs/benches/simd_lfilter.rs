...
//! (1 second @ 48 kHz = 48000 samples) which is the actual workload
//! driver for `integrated_loudness`.
//!
...
/// Generic kernel body — transcribed from
...
fn bench_k_weight_chain(c: &mut Criterion) {
  // The actual K-weighting workload: chain HS → HP over a 1 s 48 kHz
  // buffer. Real-world driver for `integrated_loudness`.
  let (hs_b, hs_a) = k_weight_hs_coeffs_48k();
  let (hp_b, hp_a) = k_weight_hp_coeffs_48k();
  let n = 48000_usize;
  let src = gen_samples(n);

  let mut group = c.benchmark_group("lfilter_k_weight_chain/n=48000");
  group.throughput(criterion::Throughput::Elements(n as u64));
